...
function* getSingleRequested({ payload }) {
    try {
        if (payload) {
            const hotel = yield call(getSingleAPI, payload)
            yield put(getSingleAction(hotel))
        } else {
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.HOTEL_REQUEST_DEL), delRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.HOTEL_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.HOTEL_REQUEST_SAVE_DATA), saveDataRequested)
}
