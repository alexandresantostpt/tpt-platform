...
function* getSingleRequested({ payload }) {
    try {
        if (payload) {
            const obj = yield call(getSingleAPI, payload)
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.PROGRAMMING_REQUEST_DEL), delRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.PROGRAMMING_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.PROGRAMMING_REQUEST_SAVE_DATA), saveDataRequested)
}
