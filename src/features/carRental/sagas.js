...
import { delAPI, saveAPI, getSingleAPI } from './api'
...
function* getSingleRequested({ payload: { id } }) {
    try {
        if (id) {
            const rent = yield call(getSingleAPI, id)
            yield put(getSingleAction(rent))
        } else {
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.CAR_RENTAL_REQUEST_DEL), delRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.CAR_RENTAL_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.CAR_RENTAL_REQUEST_SAVE_DATA), saveDataRequested)
}
