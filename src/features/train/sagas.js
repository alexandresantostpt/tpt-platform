...
function* getSingleRequested({ payload: { id } }) {
    try {
        if (id) {
            const train = yield call(getSingleAPI, id)
            yield put(getSingleAction(train))
        } else {
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.TRAIN_REQUEST_DEL), delRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.TRAIN_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.TRAIN_REQUEST_SAVE_DATA), saveDataRequested)
}
