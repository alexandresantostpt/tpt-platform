...
function* getSingleRequested({ payload: { id } }) {
    try {
        if (id) {
            const aerial = yield call(getSingleAPI, id)
            yield put(getSingleAction(aerial))
        } else {
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.AERIAL_REQUEST_DEL), delRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.AERIAL_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.AERIAL_REQUEST_SAVE_DATA), saveDataRequested)
}
