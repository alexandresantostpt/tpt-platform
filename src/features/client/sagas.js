...
function* saveDataRequested({ payload }) {
    try {
        ...
        const newClient = yield call(saveAPI, payload)
        yield put(saveDataAction(newClient))
        if (id) {
            yield Toast.success({
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.CLIENT_REQUEST_DEL), delRequested)
}

function* watchGetListRequest() {
    yield takeLatest(getAction(actions.CLIENT_REQUEST_GET_LIST), getListRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.CLIENT_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.CLIENT_REQUEST_SAVE_DATA), saveDataRequested)
}

function* watchResetRequest() {
    yield takeLatest(getAction(actions.CLIENT_REQUEST_RESET), resetRequested)
}

function* watchSearchRequest() {
    yield debounce(times.HALF_SECOND, getAction(actions.CLIENT_REQUEST_SEARCH), searchRequested)
}
