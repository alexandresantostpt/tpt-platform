...
function* getSingleRequested({ payload: { id } }) {
    try {
        if (id) {
            const cruise = yield call(getSingleAPI, id)
            yield put(getSingleAction(cruise))
        } else {
...
function* watchDelRequest() {
    yield takeLatest(getAction(actions.CRUISE_REQUEST_DEL), delRequested)
}

function* watchGetSingleRequest() {
    yield takeLatest(getAction(actions.CRUISE_REQUEST_GET_SINGLE), getSingleRequested)
}

function* watchSaveDataRequest() {
    yield takeLatest(getAction(actions.CRUISE_REQUEST_SAVE_DATA), saveDataRequested)
}
