...
const reducers = handleActions(
    {
        [delAction]: (state, { payload }) => {
            ...
            const newList = List(_listFilter.toJS().filter(item => item.userId !== payload.userId))
            return state.merge({
                _listFilter: newList,
                list: newList,
                obj: null
        ...
        [saveDataAction]: (state, { payload }) => {
            const { _listFilter } = state
            const { _id, __v } = payload.agency
            const listJS = _listFilter.toJS()
            let newList = null
            if (__v > 0) {
                const findIndex = listJS.findIndex(item => item.agency._id === _id)
        ...
        [searchAction]: (state, { payload }) => {
            ...
            const options = {
                keys: ['statusToTable', 'socialNameToTable', 'nameToTable', 'lastAccessToTable'],
                threshold: 0.3
            }
